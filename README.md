...
| Async work | Backend publishes Celery tasks to RabbitMQ; worker execution planned |
...
| Retrieval | Milvus running in local Compose; chunking/indexing planned |
...
| Meeting processing worker | Planned | RabbitMQ -> Celery worker -> processed JSON |
| Voice processing | Planned | Audio -> VAD -> WeSpeaker -> ASR -> transcript entries |
| JSON intelligence | Planned | Transcript -> LLMProvider -> validated `meeting_intelligence_result` |
| RAG chat | Planned | Question -> JSON chunks -> rerank -> LLM answer with citations |

## Deployment Profiles

| Profile | Model Strategy | Notes |
|---|---|---|
| Local dev | Local ASR, WeSpeaker, embedding, rerank, Ollama fallback | Lower quality for JSON generation |
| Hybrid | Local voice/embedding/rerank, private LLM endpoint | Recommended development path |
| Production | API/private providers with local fallback where useful | Requires auth, retention, audit, and observability hardening |
...
- Worker execution, model providers, and RAG chat are planned but not implemented yet.
- The `meeting_intelligence_result` JSON shape is a planning contract, not a finalized API schema.
- Model names and provider choices should remain configurable through environment variables.
