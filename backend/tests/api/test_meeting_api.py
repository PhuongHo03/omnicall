...
class FakeOperationalLogService:
    def __init__(self) -> None:
        self.events: list[dict] = []
        self.cleared_meeting_ids: list[str] = []

    def emit(self, **event) -> None:
        self.events.append(event)

    def clear_by_meeting(self, meeting_id: str) -> int:
        self.cleared_meeting_ids.append(meeting_id)
        before = len(self.events)
        self.events = [event for event in self.events if event.get("meeting_id") != meeting_id]
...
class FailingQueueProvider:
    def revoke_meeting_processing(self, *, meeting_ids: list[str]) -> dict:
        return {"requested": len(meeting_ids), "revoked": 0, "status": "failed", "error": "BrokerUnavailable"}


class MeetingApiTestCase(unittest.TestCase):
    def setUp(self) -> None:
        self.storage = FakeStorageProvider()
        self.operational_logs = FakeOperationalLogService()
        ...
        app.dependency_overrides[get_operational_log_service] = lambda: self.operational_logs
    ...
    def test_text_meeting_upload_is_rejected_by_voice_only_allowlist(self) -> None:
        create_response = self.client.post(
            "/api/meetings",
            headers=self.headers,
            json={"title": "Voice only meeting"},
        )
        self.assertEqual(create_response.status_code, 201, create_response.text)
        meeting_id = create_response.json()["id"]
        ...
        upload_response = self.client.post(
            ...
            headers={**self.headers, "Idempotency-Key": "text-upload"},
            files={"file": ("meeting.txt", b"00:00 Alice: Store one JSON result.\n", "text/plain")},
        )

        self.assertEqual(upload_response.status_code, 400)
        self.assertEqual(upload_response.json()["code"], "unsupported_file_extension")
    ...
    def test_delete_keeps_meeting_when_vector_cleanup_fails(self) -> None:
        create_response = self.client.post(
            "/api/meetings",
            headers=self.headers,
            json={"title": "Vector cleanup failure"},
        )
        self.assertEqual(create_response.status_code, 201, create_response.text)
        meeting_id = create_response.json()["id"]

        app.dependency_overrides[get_vector_provider] = lambda: FailingVectorProvider()
        delete_response = self.client.delete(f"/api/meetings/{meeting_id}", headers=self.headers)

        self.assertEqual(delete_response.status_code, 503, delete_response.text)
        self.assertEqual(delete_response.json()["code"], "vector_index_unavailable")
        get_response = self.client.get(f"/api/meetings/{meeting_id}", headers=self.headers)
        self.assertEqual(get_response.status_code, 200, get_response.text)

    def test_delete_keeps_meeting_when_queue_revoke_fails(self) -> None:
        create_response = self.client.post(
            "/api/meetings",
            headers=self.headers,
            json={"title": "Queue revoke failure"},
        )
        self.assertEqual(create_response.status_code, 201, create_response.text)
        meeting_id = create_response.json()["id"]

        app.dependency_overrides[get_processing_queue_provider] = lambda: FailingQueueProvider()
        delete_response = self.client.delete(f"/api/meetings/{meeting_id}", headers=self.headers)

        self.assertEqual(delete_response.status_code, 503, delete_response.text)
        self.assertEqual(delete_response.json()["code"], "processing_queue_unavailable")
        get_response = self.client.get(f"/api/meetings/{meeting_id}", headers=self.headers)
        self.assertEqual(get_response.status_code, 200, get_response.text)
