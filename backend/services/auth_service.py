...
class AuthService:
    ...
    def register(self, request: AuthRegisterRequest) -> AuthSessionResponse:
        ...
        response = self._create_session_response(user)
        self.session.commit()
        return response
    ...
    def login(self, request: AuthLoginRequest) -> AuthSessionResponse:
        ...
        self.audit.create(
            ...
            resource_type="user",
            resource_id=user.id,
            metadata={"role": _normalize_product_role(user.role)},
        )
        response = self._create_session_response(user)
        self.session.commit()
        return response
    ...
    def me(self, context: CurrentUserContext) -> MeResponse:
        ...
        return MeResponse(account=_account_response(user))

    def _create_session_response(self, user: User) -> AuthSessionResponse:
        ...
        return AuthSessionResponse(
            ...
            account=_account_response(user),
        )
