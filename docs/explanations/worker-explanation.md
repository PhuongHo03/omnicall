...
| `evidence.citations` | Canonical transcript/time-range evidence records with deterministic quotes copied from transcript segments |
...
*Document reflects project state after **Phase 25 Codex-Style Hierarchical Intelligence Extraction**. Worker processing uses full transcript evidence plus persisted bounded windows, retryable local extraction, unified global reduction, and generation-aware retrieval indexing.*
