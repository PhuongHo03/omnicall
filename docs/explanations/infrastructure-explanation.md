...
└── infras/
    ├── model-init/
    │   └── download_models.py        <- One-shot ASR/diarization/rerank model snapshot bootstrap
    ├── docker-exporter/
...
| Redis admin metrics cache | First request `cache.hit=false`, immediate second request `cache.hit=true` |
| Container metrics in admin response | `container_cpu` and `container_memory` both `ok` with 10 series |
...
*Document reflects project state after Phase 7 hardening verification on **2026-06-17**. Frontend, backend, worker, gateway, PostgreSQL, Redis, RabbitMQ, MinIO, Milvus, Prometheus, Docker stats exporter, NGINX exporter, PostgreSQL exporter, Redis exporter, RabbitMQ Prometheus plugin, MinIO metrics, backend `/metrics`, admin `/api/admin/metrics`, Redis admin metrics cache, Compose Ollama, `ollama-init`, `model-init`, `ollama_data`, `model_cache`, backend bearer sessions, `AUTH_SESSION_TTL_HOURS`, `0007_normalize_product_roles`, Admin/User database role constraints, account file storage, admin meeting-session deletion, text transcript extraction, idempotent ffmpeg audio preprocessing, local energy VAD, faster-whisper ASR runner, WeSpeaker diarization runner, SentenceTransformers rerank runner, voice safe failure states, LLM provider configuration, LLM analysis normalization, derived transcript/insight/chunk indexes, Ollama text embeddings, Milvus REST vector upsert/search with dimension recovery, Ollama guardrail provider boundary, transcript/input/context/output guardrails, evidence-guarded meeting chat history, chat citations, frontend auth/file/admin UI, admin dashboard UI, and worker safety tests are wired in Compose.*
